...
if 'df' in st.session_state :
    ...
    total_por_año_mes = df.groupby([df['Fecha'].dt.year.rename('año'), # Renombrar la columna para que no se pierda en el df final 
                                    df['Fecha'].dt.month.rename('mes')]).size().reset_index(name='cantidad de implicados') # Contar las ocurrencias y resetear el índice para obtener un df plano
    total_por_año_mes['mes'] = pd.to_datetime(total_por_año_mes['mes'], format='%m').dt.strftime('%B') # Convertir el número de mes a nombre de mes en español 

    #Graficar utilizando Plotly Express
    fig = px.line(total_por_año_mes, x='mes', y='cantidad de implicados', color='año', # Definir los datos para el gráfico de líneas 
                title = 'Distribución de implicados en los Accidentes por Mes y Año', 
                color_discrete_sequence=px.colors.sequential.Viridis)

    #Ajustar tamaño grafico
    fig.update_layout(height=600, width=1100) # Ajustar el tamaño del gráfico

    #Ajustar tamaño de la fuentes
    fig.update_xaxes(tickfont=dict(size=18)) # Ajustar el tamaño de la fuente del eje x
    fig.update_yaxes(tickfont=dict(size=18)) # Ajustar el tamaño de la fuente del eje y
    fig.update_layout(title_font_size=24) # Ajustar el tamaño de la fuente del título
    fig.update_layout(legend_title_font_size=16) # Ajustar el tamaño de la fuente del título de la leyenda

    # Mostrar el gráfico en la primera columna
    col1.plotly_chart(fig, use_container_width=True) # use_container_width=True es para que ocupe todo el ancho del contenedor

    #?------------------- Mostramos las tarjetas por lesividad ---------------#

    lesividad_counts = df['Lesividad'].value_counts() # Contar los valores de la columna 'Lesividad'

    col2.markdown(estilos.tarjeta(lesividad_counts),unsafe_allow_html=True) # Mostrar las tarjetas en la segunda columna
    ...
    with edades:
        ...
        edad_order = ['0-17', '18-29', '30-39', '40-49', '50-59', '60-64', '65-69', '70-74', '+74'] # Orden de los grupos de edad

        # Agrupar por tipo de lesividad y grupo de edad y contar las ocurrencias
        total_por_Lesividad_y_edad = df.groupby(['Lesividad', 'Edad']).size().reset_index(name='cantidad') # Contar las ocurrencias y resetear el índice
        ...
        fig = px.bar(total_por_Lesividad_y_edad, x='Edad', y='cantidad', color='Lesividad',
                    ...
                    barmode='stack', category_orders={'Edad': edad_order},
                    color_discrete_sequence=px.colors.sequential.Viridis)
    ...
    with fallecidos:
        ...
        fig = make_subplots(rows=1, cols=2, specs=[[{'type': 'domain'}, {'type': 'domain'}]]) # Crear subplots con 1 fila y 2 columnas para los dos gráficos

        fig.add_trace(go.Pie(labels=fallecidos_tpPersona['Implicado'].values, 
                            values=fallecidos_tpPersona['cantidad'].values,
                            name='', textinfo='label+percent+value', showlegend=False, hole=.3, # hole para agregar un agujero en el centro
                            marker=dict(colors=paleta_colores)), 1, 1)  # Añadir el gráfico de pastel a la primera fila y primera columna
    ...
    with positivos:
        ...
        ] # no consideramos negativo alcohol y negativo droga ya que no nos interesa en este caso
        ...
        conteo_positivos_lesividad = df.groupby(['Positivos', 'Lesividad']).size().reset_index(name='Cantidad') # esto se hace porque se quiere ver la relación entre positivos y lesividad


        # Crear la figura con subplots
        fig = make_subplots(rows=1, cols=3, specs=[[{'type': 'domain'}, {'type': 'domain'}, {'type': 'domain'}]]) # Crear subplots con 1 fila y 3 columnas
        ...
        def add_pie_trace(fig, data, row, col, name):
            fig.add_trace(
                ...
                row, col # Añadir la traza a la fila y columna correspondiente
            )
        ...
        for i, positivo in enumerate(resultados):
            ...
            add_pie_trace(fig, data, 1, i + 1, positivo) # Añadir la traza a la fila 1 y la columna i+1
